  ...
  return (
    <div className="w-full max-w-5xl mx-auto pb-32 space-y-6 animate-fade-in px-2">
      ...
      <section className="bg-slate-950 p-6 md:p-10 rounded-[44px] relative overflow-hidden flex flex-col md:flex-row items-center gap-8 group shadow-2xl border border-white/5">
         ...
         <div className="relative shrink-0">
            ...
            <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 bg-white px-3 py-1 rounded-full shadow-lg border border-slate-100">
               <span className="text-[7px] font-black text-slate-900 uppercase tracking-widest whitespace-nowrap italic">STREAK NODE</span>
            </div>
         </div>

         <div className="flex-grow space-y-5 text-center md:text-left">
            <div className="space-y-1">
               <h2 className="text-2xl md:text-3xl font-black text-white uppercase italic tracking-tighter leading-none">The Inferno Hub.</h2>
               <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Maintain your daily production streak for massive PKR multipliers.</p>
            </div>
            
            <div className="space-y-3">
               <div className="flex justify-between items-end px-1">
                  <span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest">Earning Consistency: {streak}% Capacity</span>
                  <span className="text-[9px] font-black text-white uppercase tracking-widest flex items-center gap-1.5"><Trophy size={12} className="text-amber-400" /> Goal: Day 7</span>
               </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
         <StatItem label="Capacity Today" val={limits.total} icon={Briefcase} color="bg-indigo-600" />
         <StatItem label="Submitted Proof" val={limits.used} icon={CheckCircle2} color="bg-emerald-600" />
         ...
         <div className="bg-white p-5 rounded-[32px] border border-slate-100 flex items-center justify-between shadow-sm">
            <div>
               <p className="text-[8px] font-black uppercase text-slate-400 mb-1 italic">Identity Plan</p>
      ...
      <div className="flex bg-white p-1.5 rounded-[28px] border border-slate-100 shadow-sm w-fit gap-1 mx-auto md:mx-0">
         <button onClick={() => setTab('work')} className={clsx("px-10 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all", tab === 'work' ? "bg-slate-900 text-white shadow-xl" : "text-slate-400")}>Active Registry</button>
         <button onClick={() => setTab('history')} className={clsx("px-10 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all", tab === 'history' ? "bg-slate-900 text-white shadow-xl" : "text-slate-400")}>Audit Logs</button>
      </div>
          ...
          <motion.div key="work" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
             {loading ? (
               <div className="col-span-full py-32 text-center flex flex-col items-center gap-4">
                  ...
                  <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Querying Work Cluster...</p>
               </div>
             ) : tasks.map((task, idx) => (
                <div key={task.id} className="relative group">
                   ...
                   {task.isLocked && task.myStatus !== 'completed' && (
                     <div className="absolute inset-0 z-10 bg-white/60 backdrop-blur-[4px] rounded-[40px] flex flex-col items-center justify-center p-8 text-center border-2 border-dashed border-indigo-100 group-hover:bg-white/80 transition-all">
                        ...
                        <h4 className="text-[11px] font-black text-slate-900 uppercase italic mb-1">Locked Slot.</h4>
                        <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed mb-4">
                           {task.lockReason === 'No Active Station' ? 'Activation Required' : 'Slot Limit Reached'}
                        </p>
                        <button onClick={() => navigate('/user/plans')} className="h-9 px-6 bg-indigo-600 text-white rounded-xl font-black text-[8px] uppercase tracking-widest shadow-lg flex items-center gap-2 active:scale-95 transition-all">
                           Upgrade station <ArrowRight size={12}/>
                        </button>
                     </div>
                   )}
                </div>
