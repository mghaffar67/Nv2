...
const RequestsManager = () => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      approvedTodayCount: data.filter(i => i.status === 'approved' && i.date === new Date().toISOString().split('T')[0]).length,
      ...
      <div className="bg-white p-3 rounded-[32px] border border-slate-100 shadow-sm space-y-4">
         <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="relative flex-grow max-w-md">
               ...
               <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full h-11 pl-11 pr-4 bg-slate-50 border border-slate-100 rounded-xl font-bold text-[10px] outline-none" placeholder="Probe identity or transaction reference..." />
            </div>
         </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
             <AnimatePresence mode="popLayout">
                {filteredData.map(item => (
                  <motion.div layout key={item.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.9 }} className="bg-white p-6 rounded-[36px] border border-slate-100 shadow-sm flex flex-col group hover:shadow-2xl hover:border-indigo-100 transition-all">
                     ...
                     <div className="space-y-4 mb-6 flex-grow">
                        ...
                        <div className="flex justify-between items-center text-[8px] font-black text-slate-500 uppercase tracking-widest px-1">
                           <span>Ref: {item.trxId || item.id?.slice(0, 8)}</span>
