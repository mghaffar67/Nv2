...
const StatCard = ({ title, value, icon: Icon, trend, color }: any) => (
  ...
  >
    ...
    <div className="flex justify-between items-start mb-4 relative z-10">
      ...
      <div className={clsx("px-2 py-1 rounded-lg text-[8px] font-black flex items-center gap-1", (trend || 0) > 0 ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600")}>
        {(trend || 0) > 0 ? <ArrowUpRight size={10}/> : <ArrowDownRight size={10}/>} {Math.abs(trend || 0)}%
      </div>
    </div>
  ...
  return (
    <div className="space-y-6 pb-24 animate-fade-in max-w-7xl mx-auto px-1">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 px-1">
        <StatCard title="Gross Revenue" value={`Rs. ${data?.summary?.revenue?.toLocaleString() || '0'}`} icon={TrendingUp} trend={12} color="text-emerald-500" />
        <StatCard title="Total Payouts" value={`Rs. ${data?.summary?.payouts?.toLocaleString() || '0'}`} icon={Wallet} trend={-5} color="text-rose-500" />
        <StatCard title="Net Ecosystem" value={`Rs. ${data?.summary?.profit?.toLocaleString() || '0'}`} icon={ShieldCheck} trend={8} color="text-indigo-500" />
        <StatCard title="Member Base" value={data?.summary?.totalMembers || '0'} icon={Users} trend={24} color="text-sky-500" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 px-1">
        <div className="lg:col-span-8 bg-white p-6 md:p-8 rounded-[44px] border border-slate-100 shadow-sm relative overflow-hidden">
          <div className="flex justify-between items-start mb-10">
            <div>
              <h3 className="text-lg font-black text-slate-900 uppercase italic">Financial Flow</h3>
              <p className="text-[9px] font-bold text-slate-400 uppercase mt-1 italic tracking-widest">Inflow vs Outflow Nodes</p>
            </div>
          </div>

          <div className="h-[350px] w-full">
            {data?.charts?.finance && data.charts.finance.length > 0 && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 px-1">
         <div className="lg:col-span-7 bg-white p-8 rounded-[44px] border border-slate-100 shadow-sm">
            ...
            <div className="h-[250px]">
                {data?.charts?.growth && data.charts.growth.length > 0 && (
                  <ResponsiveContainer width="100%" height="100%">
         ...
         <div className="lg:col-span-5 bg-white p-8 rounded-[44px] border border-slate-100 shadow-sm">
            <div className="mb-8 flex justify-between items-center">
                <div>
                  <h3 className="text-sm font-black text-slate-900 uppercase italic">Membership Mix</h3>
                  <p className="text-[9px] font-bold text-slate-400 uppercase mt-1 italic tracking-widest">Plan Tier Distribution</p>
                </div>
            </div>
            <div className="h-[250px]">
                {data?.charts?.plans && data.charts.plans.length > 0 && (
                  <ResponsiveContainer width="100%" height="100%">
                    <RePieChart>
                        <Pie
