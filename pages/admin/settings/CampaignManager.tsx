...
const CampaignManager = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEditing(null);
      fetchCampaigns();
      alert("Campaign Deployed to Production Stream.");
    } catch (err: any) {
      alert(err.message);
    } finally {
  ...
  const deleteCampaign = async (id: string) => {
    if (!window.confirm("Remove this campaign node?")) return;
    try {
  ...
  const startNew = () => {
    setEditing({
      ...
      targetAudience: 'all',
      specificUserIds: '', // New Field
      validFrom: new Date().toISOString().split('T')[0], // New Field
      validUntil: '', // New Field
    ...
    <div className="space-y-8 animate-fade-in pb-24">
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 border-b border-slate-50 pb-8 px-2">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">
            Campaign <span className="text-indigo-600">Architect.</span>
          </h1>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px] mt-4 flex items-center gap-2 italic">
            <Target size={14} className="text-indigo-500" /> Targeted Popups & Global Broadcast Node
          </p>
        </div>
        <button onClick={startNew} className="h-14 px-10 bg-slate-950 text-white rounded-[24px] font-black text-[11px] uppercase tracking-[0.2em] shadow-2xl flex items-center gap-3 active:scale-95 transition-all">
          <Plus size={20} className="text-sky-400" /> Initialize Campaign
        </button>
      </header>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
             ...
             {editing ? (
               ...
               >
                  <div className="flex justify-between items-center">
                     <h3 className="text-2xl font-black text-slate-900 uppercase italic">Configure Node.</h3>
                     <button type="button" onClick={() => setEditing(null)} className="p-3 bg-slate-50 rounded-full hover:bg-rose-500 transition-all"><X size={24}/></button>
                  </div>
                  ...
                  <div className="space-y-8">
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-2">
                           <label className="text-[10px] font-black text-slate-400 uppercase ml-4 italic">Banner URL</label>
                           <input value={editing.imageUrl} onChange={e => setEditing({...editing, imageUrl: e.target.value})} className="w-full h-14 px-7 bg-slate-50 border border-slate-100 rounded-[28px] font-mono text-[9px]" placeholder="https://..." />
                        </div>
                     </div>

                     <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase ml-4 italic">Broadcast Message</label>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="space-y-2">
                           <label className="text-[10px] font-black text-slate-400 uppercase ml-4 flex items-center gap-2"><Target size={12}/> Target Class</label>
                           <select value={editing.targetAudience} onChange={e => setEditing({...editing, targetAudience: e.target.value})} className="w-full h-14 px-6 bg-slate-50 border border-slate-100 rounded-[28px] font-black text-[10px] uppercase outline-none appearance-none">
                              <option value="all">Global Reach</option>
                              <option value="free_users">Non-Station Nodes</option>
                              <option value="paid_users">Station Partners</option>
                              <option value="specific">Specific User IDs</option>
                           </select>
                        </div>
                        <div className="md:col-span-2 space-y-2">
                           <label className="text-[10px] font-black text-slate-400 uppercase ml-4 flex items-center gap-2"><UserCheck size={12}/> Specific IDs (Comma Separated)</label>
                           <input disabled={editing.targetAudience !== 'specific'} value={editing.specificUserIds} onChange={e => setEditing({...editing, specificUserIds: e.target.value})} className="w-full h-14 px-7 bg-slate-50 border border-slate-100 rounded-[28px] font-mono text-[9px] disabled:opacity-30" placeholder="USR-123, USR-456..." />
                        </div>
                     </div>

                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-2">
                           <label className="text-[10px] font-black text-slate-400 uppercase ml-4 flex items-center gap-2"><Clock size={12}/> Validity (Start)</label>
                        ...
                        <div className="space-y-2">
                           <label className="text-[10px] font-black text-slate-400 uppercase ml-4 flex items-center gap-2"><Clock size={12}/> Expiry (End)</label>
                  ...
                  <div className="pt-6">
                     <button type="submit" disabled={saveLoading} className="w-full h-18 bg-slate-950 text-white rounded-[32px] font-black text-[12px] uppercase tracking-[0.3em] shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-4">
                        {saveLoading ? <Loader2 className="animate-spin" size={24}/> : <><BadgeCheck size={24} className="text-sky-400" /> Synchronize Stream</>}
                     </button>
                  </div>
               </motion.form>
             ) : (
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {campaigns.map(c => (
                    <div key={c.id} className="bg-white p-8 rounded-[48px] border border-slate-100 shadow-sm hover:border-indigo-100 hover:shadow-xl transition-all group relative overflow-hidden">
                       ...
                       <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-6 italic leading-relaxed">Scope: {c.targetAudience?.toUpperCase()}</p>
                       <div className={clsx("inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border transition-colors", c.isActive ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-slate-50 text-slate-400 border-slate-100")}>
                          <div className={clsx("w-1.5 h-1.5 rounded-full", c.isActive ? "bg-emerald-500 animate-pulse" : "bg-slate-400")} /> {c.isActive ? 'Online' : 'Hibernated'}
                       </div>
                    </div>
                  ))}
               </div>
        ...
        <div className="xl:col-span-4 sticky top-24">
                   ...
                   <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 bg-slate-950/70 backdrop-blur-md z-50 flex items-center justify-center p-5">
                      <motion.div initial={{ scale: 0.8, y: 30, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} className="bg-white w-full rounded-[44px] overflow-hidden shadow-2xl border border-white">
                         ...
                         <div className="p-8 text-center space-y-4">
                            <h4 className="text-xl font-black text-slate-900 uppercase italic leading-none">{editing.title}</h4>
                            <p className="text-[10px] font-bold text-slate-500 leading-relaxed uppercase">{editing.bodyText}</p>
                            <button className="w-full h-12 bg-slate-950 text-white rounded-2xl font-black text-[9px] uppercase tracking-widest">{editing.btnText || 'Explore Node'}</button>
                         </div>
                      </motion.div>
                   </motion.div>
                 )}
           ...
           <div className="mt-8 text-center"><p className="text-[9px] font-black text-slate-400 uppercase italic tracking-widest flex items-center justify-center gap-2"><ShieldCheck size={14} className="text-indigo-600" /> Authorized Visual Probe Active</p></div>
        </div>
      </div>
    </div>
  );
