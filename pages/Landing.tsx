    ...
    <div className="overflow-hidden bg-[#f8f9fb]">
        ...
        <div className="max-w-7xl mx-auto w-full px-4 md:px-6 py-8 md:py-24">
          <div className="flex flex-col-reverse md:flex-row items-center gap-8 md:gap-20">
            ...
            >
              ...
              <p className="text-slate-400 text-xs md:text-lg font-medium mb-8 md:mb-10 max-w-xl leading-relaxed mx-auto md:mx-0 uppercase tracking-widest opacity-80">
                {heroSubtitle}
              </p>
      ...
      <section className="py-16 md:py-24 px-4 md:px-6 bg-slate-50 border-y border-slate-100">
         <div className="max-w-7xl mx-auto text-center mb-10 md:mb-16">
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tighter uppercase italic">Earning Nodes.</h2>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.3em] mt-3 md:mt-4">Select your operational tier</p>
         </div>
      ...
      <section className="py-16 md:py-24 bg-white overflow-hidden">
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tighter uppercase italic">Member Intel.</h2>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.3em] mt-3">Verified network feedback</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {config.appearance.reviews.map((review, i) => (
              <motion.div 
                key={i}
                whileHover={{ y: -5 }}
                className="bg-slate-50 p-6 md:p-8 rounded-[30px] md:rounded-[40px] border border-slate-100 relative group"
              >
                <Quote size={40} className="absolute top-6 right-6 text-slate-200 group-hover:text-indigo-100 transition-colors" />
                <div className="flex gap-1 mb-4">
                  {Array.from({ length: review.rating }).map((_, j) => (
                    <Star key={j} size={12} className="fill-amber-400 text-amber-400" />
                  ))}
                </div>
                <p className="text-slate-600 text-sm md:text-base font-medium mb-6 italic leading-relaxed">
                  "{review.comment}"
                </p>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-black text-slate-900 text-xs uppercase">{review.name}</p>
                    <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Verified Member</p>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <section className="py-20 px-4">
        <div className="max-w-4xl mx-auto bg-slate-900 rounded-[40px] md:rounded-[60px] p-8 md:p-16 text-center text-white relative overflow-hidden shadow-2xl">
          ...
          <p className="text-slate-400 text-xs md:text-sm font-bold uppercase tracking-widest mb-10 max-w-lg mx-auto leading-relaxed relative z-10">Join thousands of Pakistanis earning daily through simple, verified digital tasks.</p>
...
const LandingStat = ({ icon: Icon, value, label, color }: any) => (
  <div className="flex flex-col items-center text-center group">
     ...
     <p className="text-lg md:text-2xl font-black text-slate-900 tracking-tight">{value}</p>
...
const PlanNode = ({ name, price, daily, icon: Icon, color, popular }: any) => (
  <Link to="/register" className={clsx(
    "bg-white p-6 md:p-10 rounded-[30px] md:rounded-[50px] border transition-all hover:scale-105 active:scale-95 relative overflow-hidden",
    popular ? "border-indigo-500 shadow-2xl ring-4 md:ring-8 ring-indigo-50" : "border-slate-100 shadow-sm"
     ...
     <div className={clsx("w-12 h-12 md:w-16 md:h-16 rounded-2xl md:rounded-3xl bg-slate-50 flex items-center justify-center mb-6 md:mb-8", color)}><Icon size={24} className="md:size-32" /></div>
     <h4 className="text-xl md:text-2xl font-black text-slate-900 uppercase tracking-tight italic">{name}</h4>
     <div className="mt-6 md:mt-8 pt-6 md:pt-8 border-t border-slate-50 space-y-3 md:space-y-4">
        <div className="flex justify-between items-end"><span className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase">Entry</span><span className="text-base md:text-lg font-black text-slate-900">Rs {price}</span></div>
        <div className="flex justify-between items-end"><span className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase">Daily</span><span className="text-base md:text-lg font-black text-green-600">Rs {daily}</span></div>
     </div>
  </Link>
);
