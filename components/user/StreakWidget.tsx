...
import { 
  Check, Zap, Loader2, Lock, Flame, Sparkles, 
  CheckCircle2, Clock
} from 'lucide-react';
...
const StreakWidget = () => {
  ...
  const handleClaim = async () => {
    ...
    } catch (err: any) { alert(err.message || "Claim Connection Failed."); } finally { setLoading(false); }
  };
    ...
    <div className="bg-white rounded-[40px] border border-slate-100 p-8 shadow-sm relative overflow-hidden mx-1 font-sans">
      <div className="flex items-center justify-between mb-10">
         <div className="flex items-center gap-4">
            ...
            <div>
               <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest italic leading-none">Hazari Hub</h3>
               <p className="text-[8px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-1.5 italic">Consistency: {streak} Days</p>
            </div>
         </div>
         <div className="text-right">
            <p className="text-[7px] font-black text-slate-300 uppercase tracking-[0.3em] mb-1 italic">Cycle Sync</p>
      ...
      <div className="flex items-center justify-between gap-2 mb-10 px-1">
         {[1, 2, 3, 4, 5, 6, 7].map((day) => {
           const isCompleted = day <= (streak % 7 || (streak > 0 ? 7 : 0));
           ...
           return (
             <div key={day} className="flex-1 flex flex-col items-center gap-3">
                ...
                <div className={clsx(
                  ...
                  isCompleted ? "bg-indigo-600 text-white border-indigo-600 shadow-lg" :
                  isCurrent ? "bg-white border-indigo-500 text-indigo-600 ring-4 ring-indigo-50 shadow-xl" :
      ...
      <AnimatePresence mode="wait">
        {claimedToday ? (
          <motion.div 
            ...
            className="w-full h-18 bg-slate-900 border border-slate-800 rounded-[32px] flex flex-col items-center justify-center gap-1 shadow-2xl relative overflow-hidden group"
          >
             <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none group-hover:opacity-10 transition-opacity"><Clock size={48} className="text-white"/></div>
             <div className="flex items-center gap-2 text-emerald-400">
                <CheckCircle2 size={16} />
                <span className="text-[11px] font-black uppercase tracking-[0.2em]">Synchronized</span>
             </div>
             <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest italic">
               Next Yield in <span className="text-white font-mono not-italic ml-1">{timeLeft.h}:{timeLeft.m}:{timeLeft.s}</span>
             </p>
          </motion.div>
        ) : (
          <motion.button 
          ...
          >
            ...
            <span className="relative z-10 flex items-center gap-3">
              {loading ? <Loader2 size={24} className="animate-spin" /> : <><Sparkles size={24} className="text-amber-400 animate-pulse" /> Claim Now</>}
            </span>
          </motion.button>
        )}
      </AnimatePresence>
