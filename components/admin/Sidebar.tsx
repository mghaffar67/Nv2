...
export const Sidebar = ({ isOpen, onToggle, isMobileOpen, onMobileClose }: any) => {
  ...
  const menu = [
    ...
    { label: 'Requests', to: '/admin/requests', icon: Inbox, badge: stats.total },
    { label: 'Members', to: '/admin/users', icon: Users },
    { label: 'Rewards', to: '/admin/rewards', icon: Trophy },
    { label: 'Daily Tasks', to: '/admin/tasks', icon: Briefcase },
    { label: 'Finance', to: '/admin/finance', icon: BarChart3 },
    { 
      label: 'System Hub', icon: ShieldAlert, 
      children: [
        { label: 'Modules', to: '/admin/advanced/modules' },
        { label: 'Campaigns', to: '/admin/advanced/campaigns' },
        { label: 'Global CMS', to: '/admin/advanced/global-cms' },
        { label: 'Pages', to: '/admin/advanced/page-editor' },
        { label: 'SEO', to: '/admin/advanced/seo' },
        { label: 'Database', to: '/admin/advanced/database' }
      ]
    },
    {
      label: 'Identity', icon: Settings,
      children: [
        { label: 'Branding', to: '/admin/settings/branding' },
        { label: 'Appearance', to: '/admin/settings/appearance' },
        { label: 'Staff Team', to: '/admin/settings/team' }
      ]
    }
  ];
    ...
    <>
      ...
      <aside className={clsx(
        ...
        <div className="px-6 flex items-center h-20 shrink-0 border-b border-slate-50 gap-3">
          <div className="w-9 h-9 bg-slate-950 rounded-xl flex items-center justify-center text-sky-400 shadow-xl shrink-0"><Shield size={18} /></div>
