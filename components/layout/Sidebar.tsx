...
export const Sidebar = ({ isOpen, onClose }: { isOpen: boolean; onClose?: () => void }) => {
  ...
  useEffect(() => {
    ...
    else if (location.pathname.startsWith('/user/work')) setOpenSub('Work Hub');
  }, [location.pathname]);
  ...
  const menuStructure = isAdmin ? [
    ...
    { 
      ...
      icon: Briefcase,
      children: [
        { label: 'Global Stream', to: '/user/work' },
        { label: 'Data Entry', to: '/user/work/data_entry' },
        { label: 'Social Media', to: '/user/work/social_media' },
        { label: 'Content Writing', to: '/user/work/content_creation' },
        { label: 'Verification', to: '/user/work/verification' }
      ]
    },
    { label: 'Bonus Hub', to: '/user/achievements', icon: Trophy },
    { label: 'My Wallet', to: '/user/wallet', icon: Wallet },
    { label: 'Network Hub', to: '/user/team', icon: Network },
